...
class ActionRunner:
    ...
    def parse_action_file(self, file_path):
        ...
        try:
            ...
            # If YAML parsing fails, try to extract information using regex
            action_data = {}
            
            # Look for action type
            type_match = re.search(r'type:\s*(\w+)', content, re.IGNORECASE)
            if type_match:
                action_data['type'] = type_match.group(1).lower()
            
            # Look for email fields
            to_match = re.search(r'to:\s*(.+)', content, re.IGNORECASE)
            if to_match:
                action_data['to'] = to_match.group(1).strip()
            
            subject_match = re.search(r'subject:\s*(.+)', content, re.IGNORECASE)
            if subject_match:
                action_data['subject'] = subject_match.group(1).strip()
            
            # Look for body content (everything after a blank line after headers)
            body_match = re.search(r'(?:\n\n|\r\n\r\n)(.+)', content, re.DOTALL)
            if body_match:
                action_data['body'] = body_match.group(1).strip()
            else:
                # If no clear body section, use the entire content as body
