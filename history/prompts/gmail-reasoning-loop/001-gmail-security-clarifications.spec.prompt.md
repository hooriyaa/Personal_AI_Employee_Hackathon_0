...
## Clarifications
### Session 2026-01-25
- Q: How should we handle `credentials.json` and `token.json` for security? → A: Use a dedicated `/secrets/` folder and ensure both files are in `.gitignore` to prevent accidental commits
- Q: What polling interval should we use to avoid hitting Gmail API rate limits? → A: 60-120 seconds interval to balance responsiveness with API rate limits
- Q: What structure should the Plan.md files follow for consistency? → A: Strict template with Steps checklist to ensure reliable parsing by the Agent
- Q: How should the system handle potentially dangerous actions like sending emails to ensure human oversight? → A: Pre-approval mechanism via `/Pending_Approval` directory before execution
...
- **FR-002**: System MUST poll for UNREAD emails with labels "URGENT" or "IMPORTANT" at 60-120 second intervals to avoid rate limits
...
- **FR-006**: System MUST create a corresponding plan file before processing each task with a strict template including a Steps checklist
...
- **FR-011**: System MUST store `credentials.json` and `token.json` in a dedicated `/secrets/` folder and ensure both files are in `.gitignore`
- **FR-012**: System MUST implement a pre-approval mechanism for potentially dangerous actions (e.g., sending emails) by generating files in `/Pending_Approval` first, with execution only occurring when moved to `/Approved`
